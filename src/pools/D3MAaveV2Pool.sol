...
contract D3MAaveV2Pool is ID3MPool {
    ...
    constructor(bytes32 ilk_, address hub_, address dai_, address pool_) {
        ...
        require(adai_         != address(0), "D3MAaveV2Pool/invalid-adai");
        require(stableDebt_   != address(0), "D3MAaveV2Pool/invalid-stableDebt");
        require(variableDebt_ != address(0), "D3MAaveV2Pool/invalid-variableDebt");
    ...
    modifier auth {
        require(wards[msg.sender] == 1, "D3MAaveV2Pool/not-authorized");
        _;
    }

    modifier onlyHub {
        require(msg.sender == hub, "D3MAaveV2Pool/only-hub");
        _;
    }
    ...
    function file(bytes32 what, address data) external auth {
        require(vat.live() == 1, "D3MAaveV2Pool/no-file-during-shutdown");
        if (what == "hub") {
            vat.nope(hub);
            hub = data;
            vat.hope(data);
        } else if (what == "king") king = data;
        else revert("D3MAaveV2Pool/file-unrecognized-param");
        emit File(what, data);
    }
    ...
    function deposit(uint256 wad) external override onlyHub {
        ...
        require(adai.scaledBalanceOf(address(this)) >= (scaledPrev + scaledAmount), "D3MAaveV2Pool/incorrect-adai-balance-received");
    }
    ...
    function withdraw(uint256 wad) external override onlyHub {
        ...
        require(dai.balanceOf(msg.sender) == prevDai + wad, "D3MAaveV2Pool/incorrect-dai-balance-received");
    }
    ...
    function exit(address dst, uint256 wad) external override onlyHub {
        ...
        require(adai.transfer(dst, amt), "D3MAaveV2Pool/transfer-failed");
    }

    function quit(address dst) external override auth {
        require(vat.live() == 1, "D3MAaveV2Pool/no-quit-during-shutdown");
        require(adai.transfer(dst, adai.balanceOf(address(this))), "D3MAaveV2Pool/transfer-failed");
    }
    ...
    function collect() external returns (uint256 amt) {
        require(king != address(0), "D3MAaveV2Pool/king-not-set");
