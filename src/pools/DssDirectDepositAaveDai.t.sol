...
//         directDepositHub.file("bar", targetBorrowRate);
...
//             uint256 targetSupply = directDepositAave.calculateTargetSupply(i * RAY / 100);
...
//         directDepositHub.reap();     // Clear out interest to get rid of rounding errors
//         assertEqInterest(getBorrowRate(), targetBorrowRate);

//         uint256 amountMinted = adai.balanceOf(address(directDepositHub));
//         assertTrue(amountMinted > 0);
//         (uint256 ink, uint256 art) = vat.urns(ilk, address(directDepositHub));
...
//         assertEq(vat.gem(ilk, address(directDepositHub)), 0);
...
//         (ink, art) = vat.urns(ilk, address(directDepositHub));
//         assertEq(ink, 0);
//         assertEq(art, 0);
//     }
...
//         assertEq(adai.balanceOf(address(directDepositHub)), 0);
//         (uint256 ink, uint256 art) = vat.urns(ilk, address(directDepositHub));
//         assertEq(ink, 0);
//         assertEq(art, 0);
//         assertEq(vat.gem(ilk, address(directDepositHub)), 0);
...
//         uint256 amountSupplied = adai.balanceOf(address(directDepositHub));
...
//         (uint256 pink, uint256 part) = vat.urns(ilk, address(directDepositHub));
...
//         (uint256 ink2, uint256 art2) = vat.urns(ilk, address(directDepositHub));
//         assertEq(directDepositHub.culled(), 1);
//         assertEq(ink2, 0);
//         assertEq(art2, 0);
//         assertEq(vat.gem(ilk, address(directDepositHub)), ink);
...
//         uint256 adaiBalance = adai.balanceOf(address(directDepositHub));
...
//         assertEqRoundingAgainst(adai.balanceOf(address(directDepositHub)), debtCeiling);    // We allow a rounding error of 1 because aTOKENs round against the user
...
//         assertEq(ink, debtCeiling);
//         assertEq(art, debtCeiling);
//         assertEqRoundingAgainst(adai.balanceOf(address(directDepositHub)), debtCeiling);
...
//         (uint256 amountSupplied,) = vat.urns(ilk, address(directDepositHub));
//         uint256 amountToBorrow = currentLiquidity;
//         pool.borrow(address(dai), amountToBorrow, 2, 0, address(this));

//         // Accumulate a bunch of interest
//         hevm.warp(block.timestamp + 180 days);
//         uint256 feesAccrued = adai.balanceOf(address(directDepositHub)) - amountSupplied;
//         currentLiquidity = dai.balanceOf(address(adai));
...
//         assertEq(ink, 0);
//         assertEq(art, 0);
//         assertGt(adai.balanceOf(address(directDepositHub)), 0);
//         assertEq(vat.dai(vow), vowDai);
...
//         hevm.warp(block.timestamp + directDepositHub.tau());

//         uint256 daiEarned = adai.balanceOf(address(directDepositHub)) - pink;
...
//         assertEq(ink, 0);
//         assertEq(art, 0);
//         assertEq(vat.gem(ilk, address(directDepositHub)), pink);
//         assertGe(adai.balanceOf(address(directDepositHub)), pink);
...
//         assertEq(vat.gem(ilk, address(end)), amountSupplied / 2);
//         assertGt(adai.balanceOf(address(directDepositHub)), amountSupplied / 2);
...
//         uint256 amountClaimed = directDepositHub.collect(tokens, uint256(-1));
...
//         uint256 amountClaimed2 = directDepositHub.collect(tokens, uint256(-1));
...
//         directDepositHub.collect(tokens, uint256(-1));
//     }
...
//         directDepositHub.exit(address(this), 100 ether);
...
//         (uint256 pink, uint256 part) = vat.urns(ilk, address(directDepositHub));
//         uint256 pbal = adai.balanceOf(address(directDepositHub));
//         assertGt(pink, 0);
//         assertGt(part, 0);
//         assertGt(pbal, 0);
...
//         (uint256 nink, uint256 nart) = vat.urns(ilk, address(directDepositHub));
//         uint256 nbal = adai.balanceOf(address(directDepositHub));
//         assertEq(nink, 0);
//         assertEq(nart, 0);
//         assertEq(nbal, 0);
...
//         uint256 pgem = vat.gem(ilk, address(directDepositHub));
//         uint256 pbal = adai.balanceOf(address(directDepositHub));
//         assertGt(pgem, 0);
//         assertGt(pbal, 0);
...
//         uint256 ngem = vat.gem(ilk, address(directDepositHub));
//         uint256 nbal = adai.balanceOf(address(directDepositHub));
//         assertEq(ngem, 0);
//         assertEq(nbal, 0);
...
//         (uint256 ink, ) = vat.urns(ilk, address(directDepositHub));
//         assertGt(ink, 0);
//         assertGt(directDepositHub.bar(), 0);

//         // Something bad happens on Aave - we need to bypass gov delay
//         directDepositMom.disable(address(directDepositHub));
...
//         (ink, ) = vat.urns(ilk, address(directDepositHub));
//         assertEq(ink, 0);
//     }
...
//         // Move down by 25%
//         directDepositHub.file("bar", getBorrowRate() * 7500 / 10000);

//         assertTrue(helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "1: 1% dev.");       // Definitely over a 1% deviation and winding room
//         assertTrue(!helper.shouldExec(address(directDepositHub), 40 * RAY / 100), "1: 40% dev.");    // Definitely not over a 40% deviation

//         directDepositHub.exec();

//         // Should be within tolerance for both now
//         assertTrue(!helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "2: 1% dev.");
//         assertTrue(!helper.shouldExec(address(directDepositHub), 40 * RAY / 100), "2: 40% dev.");

//         // Target 2% up
//         directDepositHub.file("bar", getBorrowRate() * 10200 / 10000);

//         // Should be outside of tolerance for 1% in the unwind direction
//         assertTrue(helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "3: 1% dev.");
//         assertTrue(!helper.shouldExec(address(directDepositHub), 40 * RAY / 100), "3: 40% dev.");

//         directDepositHub.exec();

//         assertTrue(!helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "4: 1% dev.");
//         assertTrue(!helper.shouldExec(address(directDepositHub), 40 * RAY / 100), "4: 40% dev.");

//         // Unwind completely by disabling the module
//         directDepositHub.file("bar", 0);

//         // Outside of both tolerance now
//         assertTrue(helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "5: 1% dev.");

//         helper.conditionalExec(address(directDepositHub), 40 * RAY / 100); // Trigger the exec here

//         // Should be outside of both tolerance, but debt is empty so should still return false
//         (uint256 daiDebt,) = vat.urns(ilk, address(directDepositHub));
//         log_named_uint("daiDebt", daiDebt);
//         assertTrue(!helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "6: 1% dev.");
//         assertTrue(!helper.shouldExec(address(directDepositHub), 40 * RAY / 100), "6: 40% dev.");

//         // Set super low bar to force hitting the debt ceiling
//         directDepositHub.file("bar", 1 * RAY / 10000);
//         directDepositHub.exec();

//         // Should be outside of both tolerance, but ceiling is hit so return false
//         assertTrue(!helper.shouldExec(address(directDepositHub), 1 * RAY / 100), "7: 1% dev.");
//         assertTrue(!helper.shouldExec(address(directDepositHub), 40 * RAY / 100), "7: 40% dev.");
//     }
// }
