...
import { D3MAaveV3TypePool, PoolLike } from "../../pools/D3MAaveV3TypePool.sol";
...
contract D3MAaveV3TypePoolTest is D3MPoolBaseTest {
    ...
    function setUp() public override {
        contractName = "D3MAaveV3TypePool";
        ...
        d3mTestPool = address(new D3MAaveV3TypePool("", hub, address(dai), address(aavePool)));
        aavePool.setPoolAdapter(d3mTestPool);
    }

    function test_sets_dai_value() public {
        assertEq(address(D3MAaveV3TypePool(d3mTestPool).dai()), address(dai));
    }

    function test_can_file_king() public {
        assertEq(D3MAaveV3TypePool(d3mTestPool).king(), address(0));

        D3MAaveV3TypePool(d3mTestPool).file("king", address(123));

        assertEq(D3MAaveV3TypePool(d3mTestPool).king(), address(123));
    }

    function test_cannot_file_king_no_auth() public {
        D3MAaveV3TypePool(d3mTestPool).deny(address(this));
        assertRevert(d3mTestPool, abi.encodeWithSignature("file(bytes32,address)", bytes32("king"), address(123)), "D3MAaveV3TypePool/not-authorized");
    }

    function test_cannot_file_king_vat_caged() public {
        FakeVat(vat).cage();
        assertRevert(d3mTestPool, abi.encodeWithSignature("file(bytes32,address)", bytes32("king"), address(123)), "D3MAaveV3TypePool/no-file-during-shutdown");
    }

    function test_deposit_calls_lending_pool_deposit() public {
        D3MTestGem(address(adai)).rely(address(aavePool));
        vm.prank(hub);
        D3MAaveV3TypePool(d3mTestPool).deposit(1);
    ...
    function test_withdraw_calls_lending_pool_withdraw() public {
        ...
        vm.prank(hub);
        D3MAaveV3TypePool(d3mTestPool).withdraw(1);
    ...
    function test_collect_claims_for_king() public {
        ...
        D3MAaveV3TypePool(d3mTestPool).file("king", king);

        D3MAaveV3TypePool(d3mTestPool).collect(address(123));
    ...
    function test_collect_no_king() public {
        assertEq(D3MAaveV3TypePool(d3mTestPool).king(), address(0));
        assertRevert(d3mTestPool, abi.encodeWithSignature("collect(address)", address(0)), "D3MAaveV3TypePool/king-not-set");
    }

    function test_redeemable_returns_adai() public {
        assertEq(D3MAaveV3TypePool(d3mTestPool).redeemable(), address(adai));
    }
    ...
    function test_exit_adai() public {
        ...
        vm.prank(hub);
        D3MAaveV3TypePool(d3mTestPool).exit(address(this), tokens);
    ...
    function test_quit_moves_balance() public {
        ...
        D3MAaveV3TypePool(d3mTestPool).quit(address(this));
    ...
    function test_assetBalance_gets_adai_balanceOf_pool() public {
        uint256 tokens = adai.totalSupply();
        assertEq(D3MAaveV3TypePool(d3mTestPool).assetBalance(), 0);
        assertEq(adai.balanceOf(d3mTestPool), 0);

        adai.transfer(d3mTestPool, tokens);

        assertEq(D3MAaveV3TypePool(d3mTestPool).assetBalance(), tokens);
        assertEq(adai.balanceOf(d3mTestPool), tokens);
    }
    ...
    function test_maxWithdraw_gets_available_assets_assetBal() public {
        ...
        assertEq(D3MAaveV3TypePool(d3mTestPool).maxWithdraw(), 0);
    }
    ...
    function test_maxDeposit_returns_max_uint() public {
        assertEq(D3MAaveV3TypePool(d3mTestPool).maxDeposit(), type(uint256).max);
    }

    function test_maxDeposit_supply_cap_active() public {
        aavePool.setSupplyCap(100);
        assertEq(D3MAaveV3TypePool(d3mTestPool).maxDeposit(), 100 ether);
    }

    function test_maxDeposit_supply_cap_adai_scaledTotalSupply() public {
        aavePool.setSupplyCap(100);
        adai.setScaledTotalSupply(50 ether);
        assertEq(D3MAaveV3TypePool(d3mTestPool).maxDeposit(), 50 ether);
    }
    ...
    function test_maxDeposit_supply_cap_over_limit() public {
        ...
        assertEq(D3MAaveV3TypePool(d3mTestPool).maxDeposit(), 0);
    }
    ...
    function test_preDebtChange_flashLoanCalled() public {
        ...
        D3MAaveV3TypePool(d3mTestPool).preDebtChange();
