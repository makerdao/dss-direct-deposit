...
import { D3MAaveV2Pool, LendingPoolLike } from "../../pools/D3MAaveV2Pool.sol";
...
contract D3MAaveV2PoolTest is D3MPoolBaseTest {
    ...
    function setUp() public override {
        ...
        d3mTestPool = address(new D3MAaveV2Pool("", hub, address(dai), address(aavePool)));
    }

    function test_sets_dai_value() public {
        assertEq(address(D3MAaveV2Pool(d3mTestPool).dai()), address(dai));
    }

    function test_can_file_king() public {
        assertEq(D3MAaveV2Pool(d3mTestPool).king(), address(0));

        D3MAaveV2Pool(d3mTestPool).file("king", address(123));

        assertEq(D3MAaveV2Pool(d3mTestPool).king(), address(123));
    }

    function test_cannot_file_king_no_auth() public {
        D3MAaveV2Pool(d3mTestPool).deny(address(this));
        assertRevert(d3mTestPool, abi.encodeWithSignature("file(bytes32,address)", bytes32("king"), address(123)), "D3MAaveV2Pool/not-authorized");
    }

    function test_cannot_file_king_vat_caged() public {
        FakeVat(vat).cage();
        assertRevert(d3mTestPool, abi.encodeWithSignature("file(bytes32,address)", bytes32("king"), address(123)), "D3MAaveV2Pool/no-file-during-shutdown");
    }

    function test_deposit_calls_lending_pool_deposit() public {
        D3MTestGem(address(adai)).rely(address(aavePool));
        vm.prank(hub);
        D3MAaveV2Pool(d3mTestPool).deposit(1);
    ...
    function test_withdraw_calls_lending_pool_withdraw() public {
        ...
        vm.prank(hub);
        D3MAaveV2Pool(d3mTestPool).withdraw(1);
    ...
    function test_collect_claims_for_king() public {
        ...
        D3MAaveV2Pool(d3mTestPool).file("king", king);

        D3MAaveV2Pool(d3mTestPool).collect();
    ...
    function test_collect_no_king() public {
        assertEq(D3MAaveV2Pool(d3mTestPool).king(), address(0));
        assertRevert(d3mTestPool, abi.encodeWithSignature("collect()"), "D3MAaveV2Pool/king-not-set");
    }

    function test_redeemable_returns_adai() public {
        assertEq(D3MAaveV2Pool(d3mTestPool).redeemable(), address(adai));
    }
    ...
    function test_exit_adai() public {
        ...
        vm.prank(hub);
        D3MAaveV2Pool(d3mTestPool).exit(address(this), tokens);
    ...
    function test_quit_moves_balance() public {
        ...
        D3MAaveV2Pool(d3mTestPool).quit(address(this));
    ...
    function test_assetBalance_gets_adai_balanceOf_pool() public {
        uint256 tokens = adai.totalSupply();
        assertEq(D3MAaveV2Pool(d3mTestPool).assetBalance(), 0);
        assertEq(adai.balanceOf(d3mTestPool), 0);

        adai.transfer(d3mTestPool, tokens);

        assertEq(D3MAaveV2Pool(d3mTestPool).assetBalance(), tokens);
        assertEq(adai.balanceOf(d3mTestPool), tokens);
    }
    ...
    function test_maxWithdraw_gets_available_assets_assetBal() public {
        ...
        assertEq(D3MAaveV2Pool(d3mTestPool).maxWithdraw(), 0);
    }
    ...
    function test_maxDeposit_returns_max_uint() public {
        assertEq(D3MAaveV2Pool(d3mTestPool).maxDeposit(), type(uint256).max);
    }
}
